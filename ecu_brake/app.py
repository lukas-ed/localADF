# Brake ECU — inline SecOC

#**Path:** `ecu_brake/app.py` (replace the existing file)

#Key changes vs. the separate-frame version:
#- Removed `send_mac_frame()` helper — there is no companion MAC frame.
#- `send_brake_status()` builds the 8-byte secured frame (`payload || freshness || MAC`) in one call to `secoc.build_secured_frame()` and sends it as a single CAN frame on `0x110`.
#- Payload reduced from 5 to 4 bytes; `tx_counter` removed (freshness counter replaces it).
#- Fault report (`0x130`) stays unauthenticated — see README for rationale.
...
class BrakeECUState:
    ...
    brake_force: int = 0
    # NOTE: tx_counter removed — the SecOC freshness counter takes its role.
...
# Per-message freshness counter (per authenticated data ID this ECU sends)
freshness_counters = {}
...
def send_brake_status():
    """Build the 4-byte application payload, wrap it in a SecOC secured
    frame (payload || freshness || truncated MAC) and send as one CAN frame."""
    payload = bytes([
    ...
    freshness = next_freshness(BRAKE_STATUS_ID)
    secured = secoc.build_secured_frame(BRAKE_STATUS_ID, freshness, payload)
    ...
    try:
        print(f"[Brake ECU] Sending secured brake status on {CHANNEL}: "
...
def send_fault_report():
    """Fault report (0x130) is unauthenticated by design — see README."""
    payload = [
...
def check_for_faults(sensor_high, sensor_low):
    if brake_state.enabled or brake_state.faults > 0:
        ...
        inputs_grounded = check_voltage_grounded(
            brake_reading.high, brake_reading.low,
            FAULT_HYSTERESIS, grounded_fault_state,
        )
...
def handle_brake_input(msg):
    """Expected input payload: byte 0 = pressed (0 or 1)."""
    if len(msg.data) < 1:
...
# =========================================================
def main():
    print("[Brake ECU] Initializing (inline SecOC profile)...")
    enable_control()
